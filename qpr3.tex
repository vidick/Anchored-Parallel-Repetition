...
\]
where the last inequality follows by taking the difference and using strong subadditivity of conditional mutual information, i.e., $\I(X_i:X_1\ldots X_{i-1}|A)_\rho\geq 0$ (see Corollary 11.9.1 in~\cite{wilde2013quantum})\tnote{This is at least as big a result as any of those that you marked as propositions earlier. Why mention it off-hand here and not also as a proposition?}. 
\end{proof}
...
\end{align*}
where in the third line we used the fact that $\P_{\bOmega \bX \bY} = \P_{\bOmega} \P_{\bX | \bOmega} \P_{\bY | \bOmega}$ and in the fourth line we used that $\P_{\bA_C \bB_C | \bX \bY} = \P_{\bA_C \bB_C | \bOmega \bX \bY}$. \tnote{These two facts should be included in a claim in Section~\ref{sec:dep-var}; see note there}

Since the event $W_C$ is determined by the random variables $(\bOmega,\bA_C,\bB_C)$ we can {condition} the state $\Xi$ on the event $W_C$ to obtain a new state
\[
...
where $\rho = \sum \lambda_j \ket{v_j}\bra{v_j}$ and the complex conjugate is taken with respect to the orthonormal basis $\{\ket{v_j}\}$.  
	Using this, for all $\bomega$\tnote{Before the register you call $ \bA_C \bB_C$ here was called $Z$}
	\begin{align}
...
\end{align*}
where the second inequality follows from \Cref{prop:divergence_chain_rule}, the third from \Cref{prop:relative_min_entropy_chain_rule2} and the last uses~\eqref{eq:claim23-1} and~\eqref{eq:claim23-2b}. 
\end{proof}

The second claim relates the reduced densities on $E_A$ of the states $\xi$  associated with different choices of $i,\br_\mi,x,y$. 
...
Applying Pinsker's inequality (Lemma~\ref{prop:pinsker}) and using that $\bY_i$ is classical\tnote{can you spell this out? It's a bit quick. There's also typos: factor $\ln 2$ missing, and $E_B$ should be $E_A$?}, we get
\begin{align}
...
\end{align} 
where the last line is by~\eqref{eq:xi-change-1}. Items 1, 3, and 4 of \Cref{lem:classical_skew} imply that \tnote{can you spell this out?}
\[
...
Let $\br_\mi = (\bomega_\mi,\ba_C,\bb_C)$, $x \in \X$, and $y \in \Y$. We can write $\xi^{E_A}_\ssxy$ and $\xi^{E_A}_\ssxp$ explicitly as \tnote{Can you spell this out?}\tnote{Is the subscript $\bomega_\mi$ on $B$ a typo? It wasn't there before}\tnote{I also didn't get how the $\gamma$s appear}
\begin{align*}
...
\end{align*}
Observe now that the states $\bigket{\wt{\Phi}_\sspy}$ and $\bigket{\wt{\Phi}_\sspp}$ introduced in \Cref{sec:states-operators} purify $\xi^{E_A}_\sspy$ and $\xi^{E_A}_\sspp$ respectively. By Uhlmann's Theorem (\Cref{thm:uhlmann}), there exists a unitary $V_{\br_\mi, y}$ acting on $E_B$ such that \tnote{This needs one more step: the first equality is doing two things at once. It makes it hard to verify}
\begin{align}
...
\end{align}
where the first inequality follows from the Fuchs-van de Graaf inequality (Eq.~\eqref{eq:fuchs-graaf}) and the second uses Jensen's inequality and Claim~\ref{claim:xi-change-y}\tnote{and conditioning on $x=\dummy$?}. Translating from inner products to Euclidean distance and applying Jensen's inequality, we get that
\[
