...
\]
for all $\ba = (\ba_1,\ldots,\ba_n) \in \A^n$ and $\bb = (\bb_1,\ldots,\bb_n) \in \B^n$. $\bA$ and $\bB$ are jointly distributed as answers produced by players using strategy $\strategy^n$\tnote{Earlier this is called $\strategy^n$. Many occurrences, I wasn't sure how you want to change it} \hnote{ah, should be $\strategy^n$ throughout. changed} when their question pair for the $i$-th game is $(\bx_i,\by_i)$. 
Since conditioned on $\bX, \bY$, $\bA$ and $\bB$ are independent of $\bD,\bM$ we have that the marginal distribution of $\bX \bY \bA \bB$ is
	...
	\]
	\tnote{Are you using both notation, $A_x^a$ and $A_x(a)$, or is the above a typo?} \hnote{changed to be consistent.}
...
For $C\subseteq [n]$ such that $\P(W_C) > 0$, define \tnote{what is $m$}\tnote{we assume $W_C$ has positive probability}\hnote{added clarification}
\begin{equation}
\label{eq:delta}
...
\tnote{The next paragraph is very imprecise and false. Could we remove it?} \hnote{I wanted to add some intuitive explanation of what the next Lemma is trying to say. what part is false? The $2^{-m}$?}
...
\begin{proof}[Proof of \Cref{lem:classical_skew}]\tnote{This proof is very hard to read. I added some clarifications; see a couple remaining questions}
\begin{enumerate}
...
\item Let $T$ denote the random variable $\bOmega$, let $\bU_i$ denote the pair $(\bX_i,\bY_i)$, and let $V$ denote the pair $(\bA_C,\bB_C)$. Since $\bU_1,\ldots,\bU_m$ are independent, even conditioned on $\bOmega$ which is a product event \hnote{do you mean random variable with product distribution, not event?}, we can apply 
...
\end{equation}
We note that $\P_{\bR_\mi | \bOmega_i \bX_i \bY_i W_C} = \P_{\bR_\mi |\bX_i \bY_i W_C}$; in other words, $\bR_\mi$ is independent of $\bOmega_i$, conditioned on $\bX_i,\bY_i,W_C$. This is because $\P(W_C | \bR_\mi \bOmega_i \bX_i \bY_i) = \P(W_C | \bR_\mi \bX_i \bY_i)$ and $\P(W_C | \bOmega_i \bX_i \bY_i) = \P(W_C | \bX_i \bY_i)$, which in turn follow from the fact that the distribution of $(\bA_C,\bB_C)$, conditioned on $(\bX,\bY)$, does not depend on $\bOmega$. \tnote{I don't understand this justification} \hnote{Updated.}

Item 1 of the Lemma, combined with \Cref{lem:trivial}, implies that $\frac{1}{m} \sum_{i \in [n] \setminus C} \| \P_{\bOmega_i | W_C} - \P_{\bOmega_i}  \| \leq \sqrt{\delta}$. Using \Cref{lem:trivial} again, we get that $\frac{1}{m} \sum_{i \in [n] \setminus C} \| \P_{\bOmega_i } \P_{\bX_i \bY_i | \bOmega_i} - \P_{\bOmega_i | W_C} \P_{\bX_i \bY_i | \bOmega_i} \| \leq \sqrt{\delta}$ and thus combined with Item 1 and the triangle inequality we have $\frac{1}{m} \sum_{i \in [n] \setminus C} \| \P_{\bOmega_i | W_C} \P_{\bX_i \bY_i | \bOmega_i W_C} - \P_{\bOmega_i | W_C} \P_{\bX_i \bY_i | \bOmega_i} \| \leq 2\sqrt{\delta}$. \hnote{spelled out the derivation more}
...
\Cref{lem:trivial} then implies that~\eqref{eq:classical-skew-2} differs at most $2\sqrt{\delta}$ from
\begin{equation}
...
\end{gather*}
Using the triangle inequality with these two inequalities we get\tnote{triangle inequality: what are the two terms?} \hnote{added}
\[
...
\]
Using the fact that $\frac{1}{m} \sum_{i \in [n] \setminus C} \| \P_{\bOmega_i | W_C} - \P_{\bOmega_i} \| \leq \sqrt{\delta}$\tnote{Again, I don't think this follows from the first item} \hnote{this was argued previously} combined with \Cref{lem:trivial} and the triangle inequality we get
\[
...
\]
Marginalizing over the random variable $\bOmega_i$, we get\tnote{This probably needs one more step} \hnote{added one more step}
\[
...
\end{equation}
where we introduce a symbol ``$\dummyx$'' for every $x \in \X$ to indicate that in the $i$-th coordinate, the $i$-th question $\bX_i$ is set to $\dummy$ with probability $\eta$ and $x$ with probability $1 - \eta$. In the case that $x = \dummy$, then $\bX_i$ is set to $\dummy$ always.


\paragraph{States.}  For all $i \in [n] \setminus C$, $\br_\mi = (\bomega_\mi,\ba_C,\bb_C)$ and $x \in \X$, for all $s \in \{x,\dummyx\}$\tnote{We should clarify what $\dummyx$ is. It's a variable that ranges over a disjoint copy of $\X$, right? Including $\dummy/\dummy$} \hnote{added explanation above}, and for all $y \in \Y$, define the (unnormalized) state
\[
...
\tnote{Are we going to use this equation and the next? If so I suggest to make them a claim, with proof. It's not obvious at first that the expectations are in the right place and one has to do the math} \hnote{added:} The following proposition relates the normalization factors $\gamma_{\br_\mi,s,y}$ in terms of the probability of obtaining answers $(\ba_C,\bb_C)$ in the strategy $\strategy^n$. 
\begin{proposition}
\label{prop:gamma}
When $s \in \X$, we have
\[
...
\]
and when $s = \dummyx$ for some $x \in \X$, then 
\[
...
\end{proposition}
\begin{proof}
Suppose that $s \in \X$. Then expanding out the definition of $ A_{\bomega_\mi,s}(\ba_C)$ and $B_{\bomega_\mi,y}(\bb_C)$ we have
\begin{align*}
