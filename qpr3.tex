...
\end{corollary}
\begin{proof}
	\tnote{Removed; you did not actually use it. Remove the proposition? ``This follows from \Cref{prop:entanglement-lb-to-value}.''} Suppose that $\eval(G^n)$ is larger than 
...
\]
for all $\ba = (\ba_1,\ldots,\ba_n) \in \A^n$ and $\bb = (\bb_1,\ldots,\bb_n) \in \B^n$. $\bA$ and $\bB$ are jointly distributed as answers produced by players using strategy $\strategy$\tnote{Earlier this is called $\strategy^n$. Many occurrences, I wasn't sure how you want to change it} when their question pair for the $i$-th game is $(\bx_i,\by_i)$. 
Since conditioned on $\bX, \bY$, $\bA$ and $\bB$ are independent of $\bD,\bM$ we have that the marginal distribution of $\bX \bY \bA \bB$ is
	...
	\]
	\tnote{Are you using both notation, $A_x^a$ and $A_x(a)$, or is the above a typo?}
...
For $C\subseteq [n]$ fixed as in the previous subsection  \tnote{what is $m$}\tnote{we assume $W_C$ has positive probability}
\begin{equation}
\label{eq:delta}
	\delta = \frac{1}{m} \left ( \log \frac{1}{\P(W_C)} + |C| \log |\A| |\B| \right)\;.
\end{equation}

\tnote{The next paragraph is very imprecise and false. Could we remove it?}
...
The following hold: \tnote{The notation with $I$ and $i$ is bad. Could we write $\frac{1}{m}\sum_i$ as in the proof? Also, the proof seems to give bounds on a random $i$ from $1$ to $m$. Is this some kind of bijection with $[n]\backslash C$?}
\begin{enumerate}
...
\]
which by definition of $\delta$ in~\eqref{eq:delta} is at most $\sqrt{\delta}$. %We conclude by observe that $\| \P_I \P_{\bOmega_i \bX_i \bY_i} - \P_I \P_{\bOmega_i \bX_i \bY_i | W_C} \| = \frac{1}{m} \sum_{i=1}^m \, \Big \| \P_{\bOmega_i \bX_i \bY_i} - \P_{\bOmega_i \bX_i \bY_i | W_C} \Big \|$.

\item Let $T$ denote the random variable $\bOmega$, let $\bU_i$ denote the pair $(\bX_i,\bY_i)$, and let $V$ denote the pair $(\bA_C,\bB_C)$. Since $\bU_1,\ldots,\bU_m$ are independent, even conditioned on $\Omega$ which is a product event, we can apply 
...
\end{equation}
We note that $\P_{\bR_\mi | \bOmega_i \bX_i \bY_i W_C} = \P_{\bR_\mi |\bX_i \bY_i W_C}$; in other words, $\bR_\mi$ is independent of $\bOmega_i$, conditioned on $\bX_i,\bY_i,W_C$. This is because the event $W_C$, when conditioned on $\bR_\mi, \bX_i, \bY_i$, is independent of $\bOmega_i$. \tnote{I don't understand this justification}
...
Next we use Item 1 of the Lemma twice\tnote{No, it doesn't apply. I think you're using item 1 once, and Lemma~\ref{lem:hol} for the $\Omega_i$. Note that you use this again later, so it could be stated separately} 
 to get $\frac{1}{m} \sum_{i=1}^m \| \P_{\bOmega_i | W_C} \P_{\bX_i \bY_i | \bOmega_i W_C} - \P_{\bOmega_i | W_C} \P_{\bX_i \bY_i | \bOmega_i} \| \leq 2\sqrt{\delta}$. Using \Cref{lem:trivial}, this implies that~\eqref{eq:classical-skew-2} differs at most $2\sqrt{\delta}$ from
\[
...
\item We insert a fresh copy of $\P_{\bX_i \bY_i | \bOmega_i}$ on both sides of the difference in each term of~\eqref{eq:classical-skew-3} to get, using Lemma~\ref{lem:trivial},
\[
...
\]
Using the triangle inequality, \tnote{triangle inequality: what are the two terms?}
\[
...
\]
Finally, using the fact that $\frac{1}{m} \sum_{i=1}^m \| \P_{\bOmega_i | W_C} - \P_{\bOmega_i} \| \leq \sqrt{\delta}$ by the first item\tnote{Again, I don't think this follows from the first item}, and marginalizing out the random variable $\bOmega_i$, we get\tnote{This probably needs one more step}
\[
...
From the operators and state specified in the strategy $\strategy$ \tnote{should be $\strategy^n$?} we define new operators and states that will be used in the analysis.
...
\end{equation}
where $\Ex_{\bX | \bomega_\mi, \bx_i}$ is shorthand for $\Ex_{\bX | \bOmega_\mi = \bomega_\mi, \bX_i = \bx_i}$. 
...
\]
Note that when $s \in \X$ the normalization factor can equivalently be expressed as\tnote{Are we going to use this equation and the next? If so I suggest to make them a claim, with proof. It's not obvious at first that the expectations are in the right place and one has to do the math}
\[
