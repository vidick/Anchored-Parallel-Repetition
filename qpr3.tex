...
where $\rho = \sum \lambda_j \ket{v_j}\bra{v_j}$ and the complex conjugate is taken with respect to the orthonormal basis $\{\ket{v_j}\}$.  
	Using this, for all $\bomega$\tnote{Before the register you call $ \bA_C \bB_C$ here was called $Z$} \hnote{changed}
	\begin{align}
	...
	\end{align}
where the second-to-last equality uses $\sum_{\bb_C} \overline{B}_{\by}(\bb_C) = \Id$, \hnote{added:} and in the last equality we use that the reduced density matrices $\Xi_\bomega^{\bY}$ and $\Xi_\bomega^{E_A}$ are equal to $\sum_{\by} \P_{\bY | \bomega}(\by) \,\, \puretomixed{\by}$ and $\sum_{\ba_C} \sqrt{A_\bomega(\ba_C)} \,\,\rho\,\, \sqrt{A_\bomega(\ba_C)}$ respectively. Taking the partial trace on both sides of~\eqref{eq:claim23-2} we get
\begin{align}
\label{eq:local-unitaries-2}
\Xi_\bomega^{\bY E_A} \preceq (|\A|\cdot |\B|)^{|C|} \,\, \Xi_{\bomega}^{\bY} \otimes \Xi_{\bomega}^{E_A}~.
\end{align}
\tnote{Is the right-hand side meant to be a definition of $\Xi_{\bomega}^{\bY}$ and $\Xi_{\bomega}^{E_A}$? The notation is confusing with what you'd obtain by directly taking the direct trace. Also, it's unclear where you put the big prefactor.}\tnote{Ok I now see it's not. Then I don't get what the last ``$=$'' means. It's missing the prefactor? } \hnote{updated}
...
Applying Pinsker's inequality (Lemma~\ref{prop:pinsker}) and then using \Cref{prop:mutual-information-classical}, \tnote{can you spell this out? It's a bit quick. There's also typos: factor $\ln 2$ missing, and $E_B$ should be $E_A$?} \hnote{updated} we get
\begin{align}
...
\end{equation}
Marginalizing over $\bOmega_i$ in Item 1 of \Cref{lem:classical_skew} we get $\Ex_I \left \| \P_{\bX_i \bY_i} - \P_{\bX_i \bY_i | W_C} \right \| \leq \sqrt{\delta}$, so we can replace the second instance of $\P_{\bX_i \bY_i}$ in~\eqref{eq:xi-change-y-1} with $\P_{\bX_i \bY_i | W_C}$ (incurring an error of $\sqrt{\delta}$) to get
%Items 1, 3, and 4 of \Cref{lem:classical_skew} imply that 
\tnote{can you spell this out?} \hnote{added more steps}
\[
...
\]
\tnote{I rewrote the above because it was really hard to follow. Could you check?} \hnote{There were some errors but I fixed them}
\tnote{Then, I ran out of steam for the rest of the proof of this claim. Could you clear it up? It's really hard to follow: you never write what equation is used to derive what, so all these ``yields'' and ``we get that'' are exhausting to trace back. For an example, compare the above to how it was in the original document.} \hnote{It's a process; compare it to our original version from arxiv...}
Using the fact that $\Ex_I \| \P_{\bOmega_i | W_C} - \P_{\bOmega_i} \| \leq \sqrt{\delta}$ (which follows from Item 1 of \Cref{lem:trivial}) and conditioning on $\bD_i = A$ (which occurs with probability $1/2$) we get
\[
...
\]
Next our goal is to exchange the expectations over $\bM_i | \bD_i = A$ and $\bY_i | \bOmega_i = (A,\bM_i)$ with an expectation over $XY$. From the definition of the variables $(D,M,X,Y)$ (see \Cref{sec:p_setup}) we see $\P_X(x) \leq \frac{1 - \eta}{\alpha - \eta} \, \P_{M|D=A}(x)$ for all $x \in \X$. 
...
\end{align*}`
which concludes the proof of~\eqref{eq:E_A_1} in \Cref{claim:xi-change-y}. The proof of~\eqref{eq:E_A_2} is similar.
%
...
\end{proof}
% and $\xi^{E_A}_\ssxp$ explicitly as 
\tnote{Can you spell this out?}\tnote{Is the subscript $\bomega_\mi$ on $B$ a typo? It wasn't there before}\tnote{I also didn't get how the $\gamma$s appear} \hnote{Added the Claim above}
%\begin{align*}
...
%\end{align*}
\Cref{clm:xi-purification} implies that the states $\bigket{\wt{\Phi}_\sspy}$ and $\bigket{\wt{\Phi}_\sspp}$ purify $\xi^{E_A}_\sspy$ and $\xi^{E_A}_\sspp$ respectively, so by Uhlmann's Theorem (\Cref{thm:uhlmann}), there exists a unitary $V_{\br_\mi, y}$ acting on $E_B$ such that \tnote{This needs one more step: the first equality is doing two things at once. It makes it hard to verify} \hnote{added}
\begin{align}
...
\end{align}
where the second line follows from the Fuchs-van de Graaf inequality (Eq.~\eqref{eq:fuchs-graaf}) the third uses Jensen's inequality, and the fourth line uses Claim~\ref{claim:xi-change-y} by conditioning on $X = \dummy$, which occurs with probability $\alpha$\tnote{and conditioning on $x=\dummy$?}\hnote{added}. Translating from inner products to Euclidean distance and applying Jensen's inequality we get
\[
